...
function CountryCases() {
  ...
  const [covidCountryDetail, setCovidCountryDetail] = useState({});

  useEffect(() => {
    (async () => {
      setLoading(true);
      let countryDetails = await fetch(
        `https://api.thevirustracker.com/free-api?countryTotal=${state.countryCode}`
      );
      let countryDetailsJSON = await countryDetails.json();
      console.log(state.countryCode);
      console.log(countryDetailsJSON.countrydata);
      setCovidCountryDetail(
        countryDetailsJSON.countrydata ? countryDetailsJSON.countrydata[0] : {}
      );
      setLoading(false);
    })();
  }, [changeCountry]);
  ...
  return (
    <Grid container>
      <Grid item xs={4}>
        ...
        <Grid container spacing={5} className={classes.formControl}>
          <Grid item xs={12}>
            <Paper
              style={{
                paddingTop: "20px",
            ...
            >
              ...
              >
                {loading
                  ? "Loading ..."
                  : Object.keys(covidCountryDetail).length > 0
                  ? covidCountryDetail?.total_cases -
                    (covidCountryDetail?.total_deaths +
                      covidCountryDetail?.total_recovered)
                  : "No Data Found"}
              </Typography>
            </Paper>
          </Grid>
